...
async function getPostThreadRecursive(
	...
	try {
		...
		const remoteUserServerBaned = (
			await FederatedHost.findByPk(remoteUser.federatedHostId)
		).blocked
				...
				for await (const mention of fediMentions) {
					...
					if (mention.href?.indexOf(environment.frontendUrl) !== -1) {
						const username = mention.href?.substring(
							`${environment.frontendUrl}/fediverse/blog/`.length,
						) as string;
		...
		try {
			if (postPetition.quoteUrl) {
				const postToQuote = await getPostThreadRecursive(
					user,
				...
				if (!postToQuote) {
					postToCreate.content += `<p>RE: ${postPetition.quoteUrl}</p>`;
				}
...
async function addTagsToPost(post: any, tags: fediverseTag[]) {
	const _res = await post.setPostTags([]);
