...
import type { fediverseTag } from "../../interfaces/fediverse/tags.js";
...
async function postToJSONLD(postId: string) {
	...
	const _conversationString = `${environment.frontendUrl}/fediverse/conversation/${post.id}`;
	...
	if (ask) {
		processedContent = `<p>${getUserName(userAsker)} asked </p> <blockquote>${
			ask.question
		}</blockquote> ${processedContent} <p>To properly see this ask, <a href="${`${environment.frontendUrl}/fediverse/post/${post.id}`}">check the post in the original instance</a></p>`;
	}
	...
	if (quotedPosts && quotedPosts.length > 0) {
		...
		quotedPosts.forEach((quotedPost: any) => {
			...
			tagsAndQuotes += `<p>RE: <a href="${postUrl}">${postUrl}</a></p>`;
			fediTags.push({
				type: "Link",
				name: `RE: ${postUrl}`,
				href: postUrl,
	...
	for await (const userId of mentions) {
		...
		const url = user.url.startsWith("@")
			? user.url
			: `@${user.url}@${environment.instanceUrl}`;
