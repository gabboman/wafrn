...
async function loadPoll(apObj: any, internalPostObject: any, _user: any) {
	let res = false;
	if (
		!apObj ||
		(apObj && apObj.anyOf === undefined && apObj.oneOf === undefined)
	) {
		return res;
	}
	try {
		...
		); // This way the remaining time will be 69 years and a few months on every update of non finish date polls hehe
		...
		if (remoteQuestions.length === questions.length) {
			questions.forEach((elem: any, index: number) => {
				elem.update({
					...
					remoteReplies: remoteQuestions[index].replies.totalItems
						? remoteQuestions[index].replies.totalItems
						: 0,
			...
			for await (const [index, question] of remoteQuestions.entries()) {
				await QuestionPollQuestion.create({
					...
					remoteReplies: question.replies.totalItems
						? question.replies.totalItems
						: 0,
					questionPollId: poll.id,
				});
			}
		...
		logger.trace({
			problem: error,
			ap: apObj,
			internalPostObject: internalPostObject,
		});
	}
