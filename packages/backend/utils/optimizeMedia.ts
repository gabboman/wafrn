...
export default async function optimizeMedia(
	...
	const fileAndExtension = options?.outPath
		? [options.outPath, ""]
		: inputPath.split(".");
	const originalExtension = fileAndExtension[1].toLowerCase();
	fileAndExtension[1] = options?.forceImageExtension
		? options.forceImageExtension
		: "avif";
	...
	switch (originalExtension) {
		...
		default: {
			...
			const conversion = await sharp(inputPath, { animated: true }).rotate();
			//.toFile(outputPath)
			if (options?.maxSize) {
