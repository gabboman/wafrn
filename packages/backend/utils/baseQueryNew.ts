...
async function getMedias(postIds: string[]) {
	...
	// if media has no type or is an image and has no width, we need to get that
	await updateMediaDataQueue.addBulk(
		// biome-ignore lint/suspicious/noDoubleEquals: <explanation>
		medias.filter((media: any) => !media.mediaType || (media.mediaType?.startsWith('image') && (!media.width || media.width == 0 ) ) ).map((media: any) => {
			return {
...
async function getUnjointedPosts(postIdsInput: string[], posterId: string) {
	...
	const medias = getMedias([...postIds, ...rewootIds]);
	const tags = getTags([...postIds, ...rewootIds]);
	...
	const postsToFullySend = allPosts.filter((post: any) => {
		...
		const userFollowsPoster =
			usersFollowedByPoster.includes(post.userId) && post.privacy === 1;
	...
	const postIdsToFullySend: string[] = postsToFullySend.map(
		(post: any) => post.id,
	);
	const postsToSend = (await postWithNotes).map((post: any) =>
		filterPost(post, postIdsToFullySend),
	);
	...
	return {
		...
		quotedPosts: (await quotedPosts).map((elem: any) =>
			filterPost(elem, postIdsToFullySend),
		),
		asks: asks,
	};
}
...
function filterPost(postToBeFilter: any, postIdsToFullySend: string[]): any {
	...
	res.ancestors = res.ancestors
		? res.ancestors.map((elem: any) => filterPost(elem, postIdsToFullySend))
		: [];
	return res;
}
