		...
		if (hostBanned) {
			...
			if (userPetition) {
				...
				if (userPetition.followers) {
					...
					if (followersPetition?.totalItems) {
				...
				if (userPetition.following) {
					...
					if (followingPetition?.totalItems) {
				...
				if (res) {
					if (res !== (await getDeletedUser())) {
						...
						if (
							...
							existingUser.remoteId = `${existingUser.remoteId}_OVERWRITTEN_ON${Date.now()}`;
							existingUser.url = `${existingUser.url}_OVERWRITTEN_ON${Date.now()}`;
				...
				if (
					...
					userPetition.attachment.length > 0
				) {
...
async function getHostFromCache(displayName: string): Promise<any> {
	let res;
	try {
		res = await FederatedHost.findByPk(
			await getFederatedHostIdFromUrl(displayName),
		);
	} catch (error) {
		logger.debug({
