...
export default function emojiRoutes(app: Application) {
	app.post(
		...
		async (req: AuthorizedRequest, res: Response) => {
			...
			const packName = file.originalname
				.replaceAll(".zip", "")
				.replaceAll(".", "")
				.replaceAll("/", "")
				.replaceAll(" ", "_");
			...
			if (existingCollection?.length > 0) {
				...
				try {
					fs.createReadStream(file.destination + file.filename)
						.on("end", async () => {
							...
							const emojinames = fs
								...
								.filter((filename) =>
									filename.toLowerCase().match(fileFormats),
								);
							...
							const _emojisCreated = await Emoji.bulkCreate(emojisToCreate);
							res.send({
								success: true,
					...
					logger.info({
						message: "Error when creating emoji pack",
						error: error,
					});
					res.send({
						success: false,
