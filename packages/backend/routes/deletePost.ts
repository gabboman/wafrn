		...
		async (req: AuthorizedRequest, res: Response) => {
			...
			try {
				...
				if (id) {
					...
					if (
						new Date(postToDelete.createdAt).getTime() < 1721437200091 ||
						req.query?.nuke
					) {
						...
						usersToSendThePost = usersToSendThePostHost.flatMap(
							(elem: any) => elem.users,
						);
					} else {
	...
	app.delete(
		...
		async (req: AuthorizedRequest, res: Response) => {
			...
			try {
				...
				const user = await User.findByPk(posterId);
				if (id) {
					const postsToDeleteUnfiltered = await Post.findAll({
						where: {
							parentId: id,
							content: "",
							userId: posterId,
					...
					const unfilteredPostIds: string[] = postsToDeleteUnfiltered.map(
						(elem: any) => elem.id,
					);
					...
					const reblogsToDelete = unfilteredPostIds.filter(
						(elem) => !postsThatAreNotReblogs.includes(elem),
					);

					if (!reblogsToDelete) {
						return res.send(true);
					}
					const objectsToSend: activityPubObject[] = reblogsToDelete.map(
						(elem) => {
							return {
								"@context": [
									`${environment.frontendUrl}/contexts/litepub-0.1.jsonld`,
