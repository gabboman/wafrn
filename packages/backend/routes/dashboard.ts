...
export default function dashboardRoutes(app: Application) {
	app.get(
		...
		async (req: AuthorizedRequest, res: Response) => {
			const level = Number.parseInt(req.query.level as string); // level of dashboard: localExplore, explore, dashboard or DMs
						...
						[Op.or]: [
							{
								id: {
									[Op.in]: dms
										.map((pst: any) => pst.postId)
										.concat(myPosts.map((pst: any) => pst.id)), //latestMentionedPosts.map((elem: any) => elem.id)
								},
								userId: {
