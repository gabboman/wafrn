...
export default function notificationRoutes(app: Application) {
	app.get(
		...
		async (req: AuthorizedRequest, res: Response) => {
			...
			const likesDate = req.query?.likesDate
				? new Date(Number.parseInt(req.query.likesDate as string))
				: new Date();
			const followsDate = req.query?.followsDate
				? new Date(Number.parseInt(req.query.followsDate as string))
				: new Date();
			const reblogsDate = req.query?.reblogsDate
				? new Date(Number.parseInt(req.query.reblogsDate as string))
				: new Date();
			const mentionsDate = req.query?.mentionsDate
				? new Date(Number.parseInt(req.query.mentionsDate as string))
				: new Date();
			const emojiReactionDate = req.query?.emojiReactionDate
				? new Date(Number.parseInt(req.query.emojiReactionDate as string))
				: new Date();
			const quotesDate = req.query?.quotesDate
				? new Date(Number.parseInt(req.query.quotesDate as string))
				: new Date();

			const newQuotes = await Quotes.findAll(
				await getQuotedPostsQuery(userId, quotesDate, Op.lt, true),
			);
			...
			const mentionedPostsId = (
				await getMentionedPostsId(userId, mentionsDate, Op.lt, true)
			...
			const newEmojiReactions = getEmojiReactedPostsId(
				userId,
				emojiReactionDate,
				Op.lt,
				true,
	...
	app.get(
		"/api/v2/notificationsCount",
		authenticateToken,
		async (req: AuthorizedRequest, res: Response) => {
			const userId = req.jwtData?.userId ? req.jwtData?.userId : "";

			//const blockedUsers = await getBlockedIds(userId)
			const startCountDate = (await User.findByPk(userId))
				.lastTimeNotificationsCheck;
			const mentionIds = await getMentionedPostsId(
				userId,
				startCountDate,
				Op.gt,
			);
			const postMentions = mentionIds.length;
			const newPostReblogs = Post.count(
				await getReblogQuery(userId, startCountDate),
			);
			const newEmojiReactions = getEmojiReactedPostsId(
				userId,
				startCountDate,
			...
			const newFollows = Follows.count(
				await getNewFollows(userId, startCountDate),
			);
			const newQuotes = Quotes.count(
				await getQuotedPostsQuery(userId, startCountDate, Op.gt),
			);
			const newLikes = (await getLikedPostsId(userId, startCountDate, Op.gt))
				.length;
			...
			const unansweredAsks = Ask.count({
				where: {
					userAsked: userId,
					answered: false,
					postId: null,
	...
	async function getMentionedPostsId(
		...
		const fullyMutedDoNotCountForMentions =
			superMutedIds.length > 0
				? (
		...
		return await PostMentionsUserRelation.findAll({
			...
			where: {
				...
				createdAt: {
					[operator]: Number.isNaN(startCountDate.getDate())
						? new Date()
						: startCountDate,
				},
			},
	...
	async function getLikedPostsId(
		userId: string,
		startCountDate: Date,
		operator: any,
		limit = false,
	...
	async function getQuotedPostsQuery(
		userId: string,
		startCountDate: Date,
		operator: any,
		limit = false,
