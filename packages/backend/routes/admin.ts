...
export default function adminRoutes(app: Application) {
	...
	app.post(
		"/api/admin/server-update",
		authenticateToken,
		adminToken,
		async (req: AuthorizedRequest, res: Response) => {
			const petitionBody: server[] = req.body;
			if (petitionBody) {
				...
				dbElements.forEach(async (elemToUpdate: any) => {
					const newValue = petitionBody.find(
						(elem) => elem.id === elemToUpdate.id,
					);
					if (newValue) {
