...
function wellKnownRoutes(app: Application) {
	// webfinger protocol
	app.get("/.well-known/host-meta", (_req: Request, res) => {
		res.send(
			...
			if (req.query?.resource) {
				...
				if (
					...
					(urlQueryResource.endsWith(environment.instanceUrl) ||
						urlQueryResource.startsWith(
							`acct:${environment.frontendUrl}/fediverse/blog/`,
						))
				) {
					const userUrl = urlQueryResource.endsWith(environment.instanceUrl)
						? urlQueryResource
								.slice(5)
								.slice(0, -(environment.instanceUrl.length + 1))
						: urlQueryResource.slice(
								`acct:${environment.frontendUrl}/fediverse/blog/`.length,
							);
