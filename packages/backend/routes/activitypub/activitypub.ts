...
function activityPubRoutes(app: Application) {
			...
			if (req.params?.url) {
				...
				if (user && !user.banned) {
					...
					if (req.query?.page) {
						...
						const pageSize = 10;
						const itemsToSend = followedUsers.slice(
							(page - 1) * pageSize,
							page * pageSize,
						);
						response = {
			...
			if (req.params?.url) {
				...
				if (user && !user.banned) {
					...
					if (req.query?.page) {
						const page = Number.parseInt(req.query.page as string);
						const pageSize = 10;
						const itemsToSend = followers.slice(
							(page - 1) * pageSize,
							page * pageSize,
						);
						response = {
	...
	app.post(
		...
		getCheckFediverseSignatureFucnction(true),
		async (req: SignedRequest, res: Response) => {
			const urlToSearch = req.params?.url
				? req.params.url
			...
			if (
				user &&
				user.url !== environment.adminUser &&
				!(await checkuserAllowsThreads(req, user))
			) {
				res.sendStatus(403);
				return;
			}
	...
	app.get("/fediverse/accept/:id", (_req: SignedRequest, res: Response) => {
		res.sendStatus(200);
	});
}
