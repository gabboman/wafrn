...
export default function userRoutes(app: Application) {
		...
		async (req: AuthorizedRequest, res: Response) => {
			...
			try {
				...
				if (req.body) {
					...
					if (req.body.description) {
						...
						userEmojis = userEmojis.concat(
							avaiableEmojis?.filter((emoji: any) =>
								req.body.description.includes(emoji.name),
							),
						);
					}
					...
					if (
						req.body.forceClassicLogo !== undefined &&
						req.body.forceClassicLogo !== null
					) {
					...
					if (req.body.defaultPostEditorPrivacy) {
						const defaultPostEditorPrivacyKey =
							"wafrn.defaultPostEditorPrivacy";
					...
					if (req.body.name) {
						...
						userEmojis = userEmojis.concat(
							avaiableEmojis?.filter((emoji: any) =>
								req.body.name.includes(emoji.name),
							),
						);
					}
	...
	app.post("/api/activateUser", async (req, res) => {
		...
		if (
			...
			validateEmail(req.body.email) &&
			req.body.code &&
			!environment.reviewRegistrations
		) {
			const user = await User.findOne({
				where: {
					email: req.body.email.toLowerCase(),
		...
		try {
			if (
				...
				if (user) {
					...
					user.activated = environment.reviewRegistrations
						? user.activated
						: true;
	...
	app.get(
		...
		async (req: AuthorizedRequest, res) => {
			...
			if (req.query?.id) {
				...
				let muted = false;
				let blocked = false;
				let serverBlocked = blog?.federatedHost?.blocked;
	...
	app.post(
		...
		async (req: AuthorizedRequest, res: Response) => {
			...
			const userAskLevel = userAskLevelDbOption
				? Number.parseInt(userAskLevelDbOption.optionValue)
				: 2;
			//
			if (
