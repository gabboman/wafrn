		...
		async (req: AuthorizedRequest, res: Response) => {
			try {
				...
				if (!postId) {
					return res
						.status(400)
						.send({ success: false, errorMessage: "No post id received" });
				}
				...
				if (!post) {
					return res
						.status(404)
						.send({ success: false, errorMessage: "Post not found" });
				}
				...
				if (!userCanSeePost) {
					return res.status(403).send({
						success: false,
						errorMessage: "You are not authorized to read this post",
					});
				}
			...
			if (req.params?.id) {
					...
					include: [
						{
							...
							attributes: [
								...
								"userId",
								[
									sequelize.fn("LENGTH", sequelize.col("descendents.content")),
									"len",
								],
								"createdAt",
								"updatedAt",
				...
				const users =
					posts?.descendents?.length > 0
						...
						: [];
				res.send({
					posts: posts?.descendents?.length > 0 ? posts.descendents : [],
					users: users,
				});
			} else {
				res.sendStatus(404);
	...
	app.post(
		...
		async (req: AuthorizedRequest, res: Response) => {
			...
			try {
				...
				const quotedPostPrivacy: number = postToBeQuoted
				...
				if (parent) {
					...
					if (userFederatesWithThreads.length === 0) {
						...
						const ancestorUrls: string[] = ancestorPostsUsers.map(
							(elem: any) => elem.urlToLower,
						);
				...
				const contentWarning = req.body.content_warning
				...
				if (req.body.medias?.length > 0) {
					...
					const _updateMediasPromises: Promise<any>[] = [];
					Media.findAll({
						where: {
							id: {
				...
				if (mentionsInPost && mentionsInPost.length > 0) {
					for (let index = 0; index < mentionsInPost.length; index++) {
						...
						if (
							...
							!mentionsToAdd.includes(elem.attribs["data-id"])
						) {
					...
					if (userFederatesWithThreads.length === 0) {
						...
						if (
							mentionedUsers.some((usr: any) =>
								usr.urlToLower.endsWith("threads.net"),
							)
						) {
							success = false;
							res.status(500);
            ...
            literal: Sequelize.literal(
              `blockedServerId IN (SELECT federatedHostId from users where id IN (${  mentionsToAdd.map(
						...
						if (mentionsInPost && mentionsInPost.length > 0) {
							for (let index = 0; index < mentionsInPost.length; index++) {
								...
								if (elem.attribs["data-id"]) {
									const userMentioned = usersMentioned.find(
										(usr: any) => elem.attribs["data-id"] === usr.id,
									);
									if (userMentioned) {
										...
										const remoteUrl = userMentioned.remoteMentionUrl
				...
				if (req.body.idPostToEdit) {
					...
					post.content_warning = contentWarning;
	...
	app.post(
		...
		async (req: AuthorizedRequest, res: Response) => {
			...
			try {
				...
				if (mentionsInPost && mentionsInPost.length > 0) {
					content = content.replaceAll(/@\w+@?[\w-\.]*/gi, (userUrl: string) =>
						userUrl.toLowerCase(),
					);
						...
						where: {
							urlToLower: {
								[Op.in]: mentionsInPost.map((elem) => {
									...
									if (urlToSearch.match(/@/g)?.length === 1) {
              ...
              `blockedServerId IN (SELECT federatedHostId from users where id IN (${  mentionsToAdd.map(
                (elem) => '"' + elem + '"'
					...
					if (dbFoundMentions && dbFoundMentions.length > 0) {
						for (const userMentioned of dbFoundMentions) {
