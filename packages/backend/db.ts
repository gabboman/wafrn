...
User.addHook("beforeSave", "userToLower", (user: any, _options: any) => {
	user.descriptionToLower = user.description
...
PostTag.addHook("beforeSave", "tagToLower", (postTag: any, _options: any) => {
		...
		if (environment.forceSync) {
			...
			const admin = {
				...
				password: await bcrypt.hash(
					environment.adminPassword as string,
			...
			const _adminUser = await User.create(admin);
			const _del = await User.create(deleted);
		}
	});
