...
export class SinglePostComponent implements OnDestroy {
  ...
  constructor(
    ...
    this.routeParamsSubscription = this.route.params.subscribe(async (data: any) => {
      ...
      console.log(data)
      const tmpPost = await this.dashboardService.getPostV2(data ? data.id : '').catch(() => {
        this.postFound = false
        this.loading = false
      })
      this.post = tmpPost ? tmpPost : []
      if (this.post && this.post.length > 0 && this.post[this.post.length - 1].descendents) {
      ...
      if (lastPostFragment) {
        ...
        this.seoService.setSEOTags(
          `Wafrn - Post by ${lastPostFragment.user.url}`,
          `Wafrn post by ${lastPostFragment.user.url}: ${this.postService.getPostContentSanitized(
            lastPostFragment.content
          )}`,
          lastPostFragment.user.url,
          this.getImage(this.post)
        )
      } else {
        this.postFound = false
      }
